...
export function renderComparisonTabsForOP(container, op) {
    _renderTabShell(container, {
        ...
        structure:   OP_PARAM_STRUCTURE,
    });
}

// ── 汎用内部関数 ──────────────────────────────────────────────────────────────

function _renderTabShell(container, { detailsHtml, tracks, identKey, paramKey, label, tabLabel = 'Voies', structure = null }) {
    container.innerHTML = `
...
function _renderComparisonTable(container, tracks, identKey, paramKey, label, structure = null) {
    const matrix    = buildComparisonMatrix(tracks, { identKey, paramKey, structure });
...
export function renderComparisonTabsForSOLTunnels(container, sol) {
    ...
    _renderTabShell(container, {
        ...
        tabLabel:    'Tunnels',
        structure:   TUNNEL_SOL_STRUCTURE,
    });
}
...
export function renderComparisonTabsForOPPlatforms(container, track, label) {
    ...
    _renderTabShell(container, {
        ...
        label,
        tabLabel:    'Quais',
        structure:   OP_PLATFORM_STRUCTURE,
    });
}
...
export function renderAllComparisonTabsForSOL(container, sol) {
    ...
    _renderMultiTabShell(container, {
        detailsHtml: renderTronconBasicProperties(sol),
        tabs: [
            { id: 'voies',   tabLabel: 'Voies',  label, items: tracks,  identKey: 'SOLTrackIdentification',  paramKey: 'SOLTrackParameter', structure: PARAM_STRUCTURE,  },
            { id: 'tunnels', tabLabel: 'Tunnels', label, items: tunnels, identKey: 'SOLTunnelIdentification', paramKey: 'SOLTunnelParameter', structure: TUNNEL_SOL_STRUCTURE,  },
        ],
    });
}
...
export function renderAllComparisonTabsForOP(container, op) {
    ...
    _renderMultiTabShell(container, {
        detailsHtml: renderOpGeneralDetails(op),
        tabs: [
            { id: 'voies',          tabLabel: 'Voies',            label, items: tracks,        identKey: 'OPTrackIdentification',        paramKey: 'OPTrackParameter', structure: OP_PARAM_STRUCTURE   },
            { id: 'quais',          tabLabel: 'Quais',            label, items: platforms,     identKey: 'OPTrackPlatformIdentification', paramKey: 'OPTrackPlatformParameter', structure: OP_PLATFORM_STRUCTURE  },
            { id: 'tunnels',        tabLabel: 'Tunnels',          label, items: trackTunnels,  identKey: 'OPTrackTunnelIdentification',   paramKey: 'OPTrackTunnelParameter', structure: TUNNEL_OP_STRUCTURE    },
            { id: 'sidings',        tabLabel: 'Voies de service', label, items: sidings,       identKey: 'OPSidingIdentification',        paramKey: 'OPSidingParameter', structure: OP_SIDING_STRUCTURE         },
            { id: 'siding-tunnels', tabLabel: 'Tunnels (VS)',     label, items: sidingTunnels, identKey: 'OPSidingTunnelIdentification',  paramKey: 'OPSidingTunnelParameter', structure: TUNNEL_OP_STRUCTURE   },
        ],
    });
}
